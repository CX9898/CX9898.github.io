...
BSA-SpMM的总流程如图所示, 将输入稀疏矩阵S进行BSA重排, 生成的重排稀疏矩阵$S_R$进行分块(Tiling), 根据分块后的块(Tiled)的密度将块分为两类: **稠密块$S_d$**和**稀疏剩余$S_s$**.
